...
def validate_all_checkpoints():
    ...
    # We always need face processing (InsightFace)
    ...
    if not os.path.exists(insight_dir):
        # We don't fail immediately on insightface if they haven't downloaded it, but let's warn
        pass
    ...
    if profile.enable_restoration:
        # CodeFormer (usually used if gfpgan isn't forced, or vice versa. Let's check both or the active one.
        # Defaulting to checking CodeFormer since it's commonly used)
